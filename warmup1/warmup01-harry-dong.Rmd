...
- Ground cinnamon, _to finish_
- Powdered sugar, _also to finish_
...
In Cartesian coordinates, if **p** = ($p_1, p_2,..., p_n)$) and **q** = ($q_1, q_2,..., q_n$) are two points in Euclidean _n_-space, then the distance (d) from **p** to **q**, or from **q** to **p** is given by the Pythagorean formula:

\begin{equation} 
  \begin{aligned}
  d({\bf p}, {\bf q}) = d({\bf q}, {\bf p}) &= \sqrt{(q_1 -   p_1)^2 + (q_2 - p_2)^2 +\cdots+ (q_n - p_n)^2} \\
...
$$||{\bf p}||= \sqrt{p_1^2 + p_2^2 + \cdots+ p_n^2} = \sqrt{{\bf p} \cdot {\bf p}} $$
